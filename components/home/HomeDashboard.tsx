 ...
 return <div className={styles.page}>
  ...
  <div className={styles.shell}>
   <main className={styles.main}>
    ...
    <section className={styles.mobileOnly}><MatchCard match={next}/><QuickTabs/></section>
    ...
    <section className={styles.teamSection}><div className={styles.sectionTitle}><h2>TÝM</h2><TeamToggle team={team} setTeam={setTeam}/></div><div className={styles.playerRail}>{players.slice(0,6).map(player=><PlayerSpotlight key={player.id} player={player}/>)}</div><Link className={styles.centerButton} href="/tymy">CELÝ TÝM <b>→</b></Link></section>
...
function MatchCard({match}:{match:NextMatch|null}){return <section className={styles.widget}><div className={styles.widgetHead}><h2>DALŠÍ ZÁPAS</h2><span>FC PPB</span></div>{match?<><div className={styles.matchTeams}><TeamMark name={match.homeTeam}/><strong>VS</strong><TeamMark name={match.awayTeam}/></div><div className={styles.matchNames}><b>{match.homeTeam}</b><b>{match.awayTeam}</b></div><div className={styles.matchWhen}>{[match.day,match.date,match.time].filter(Boolean).join(" / ")}</div>{match.venue&&<p className={styles.venue}>{match.venue}</p>}<Link className={styles.widgetButton} href="/zapasy">DETAIL ZÁPASU →</Link></>:<div className={styles.noData}>Další zápas zatím není v rozpisu APF.</div>}</section>}
...
function ScorerCard({scorer}:{scorer:TopScorer|null}){return <section className={`${styles.widget} ${styles.scorer}`}><div className={styles.widgetHead}><h2>NEJLEPŠÍ STŘELEC</h2></div>{scorer?<><img src={`/images/${scorer.id}.jpg`} alt={scorer.name}/><div><h3>{scorer.name}</h3><p><b>{scorer.goals} GÓLŮ</b> / {scorer.matches} ZÁPASŮ</p><Link className={styles.widgetButton} href={`/hrac/${scorer.id}`}>PROFIL HRÁČE →</Link></div></>:<div className={styles.noData}>Statistiky střelců zatím nejsou dostupné.</div>}</section>}
...
function PlayerSpotlight({player}:{player:SquadPlayer}){return <Link href={`/hrac/${player.id}`} className={styles.playerCard}><div className={styles.playerWatermark}>PPB</div><img src={player.imageUrl||`/images/${player.id}.png`} alt={player.name}/><div className={styles.playerInfo}><span>#{player.shirtNumber??"—"}</span><h3>{player.name}</h3><p>{player.position==="goalkeeper"?"BRANKÁŘ":"HRÁČ"}</p></div></Link>}
